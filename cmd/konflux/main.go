...
const (
	...
	gitHubDir                 = ".github"
	tektonDir                 = ".tekton"
	autoGeneratedHeader       = "# Generated by openshift-pipelines/hack. DO NOT EDIT."
	autoGeneratedHeaderGitHub = "# Generated for Release %s by openshift-pipelines/hack. DO NOT EDIT."
)
...
func main() {
	...
	for _, configFile := range configFiles {
		...
		if err := cleanupAutogenerated(context.Background(), konfluxDir, autoGeneratedHeader); err != nil {
	...
	for _, repo := range config.Repos {
		...
		for _, branch := range repo.Branches {
			...
			for _, v := range branch.Versions {
				...
				if err := cleanupAutogenerated(ctx, filepath.Join(checkoutDir, tektonDir), autoGeneratedHeader); err != nil {
					return err
				}
				//Generate Tekton
				...
				if err := generateGitHubWorkflow(ctx, branch, filepath.Join(checkoutDirMain, gitHubDir)); err != nil {
					log.Fatalln(err)
				}
...
func generateKonfluxApplication(application k.Application) error {
	...
	err := cleanupAutogenerated(context.Background(), konfluxDir, autoGeneratedHeader)
	if err != nil {
		return err
	}
...
func generateGitHubWorkflow(ctx context.Context, branch k.Branch, target string) error {
	repo := branch.Repository
	log.Printf("Generate github manifests in %s\n", target)
	if err := os.MkdirAll(filepath.Join(target, "workflows"), 0o755); err != nil {
		return err
	}
	if err := cleanupAutogenerated(ctx, target, fmt.Sprintf(autoGeneratedHeaderGitHub, repo.Application.Version.Version)); err != nil {
		return err
	}
	filename := fmt.Sprintf("auto-merge-%s.yaml", branch.Name)
	if err := generateFileFromTemplate("auto-merge.yaml", branch, filepath.Join(target, "workflows", filename)); err != nil {
		return err
	}
	if repo.Upstream != "" {
...
func cleanupAutogenerated(ctx context.Context, dir string, header string) error {
	log.Printf("Cleaning up %s\n", dir)
	if out, err := run(ctx, dir, "bash", "-c", "grep -l -r '"+header+"' ."); err != nil {
